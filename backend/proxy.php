...
if (count($datosGuardados) > 0) {		
	$fechaHoraLimite = $datosGuardados[0]["DateTime"] + 28800; //Eight hours, 60 seg * 60 min * 8 hour
	//$fechaHoraLimite = $datosGuardados[0]["DateTime"] + 18000; //Five hours, 60 seg * 60 min * 5 hour
	//$fechaHoraLimite = $datosGuardados[0]["DateTime"] + 10800; //Three hours, 60 seg * 60 min * 3 hour
}
...
if ($fechaHoraLimite > $fechaHoraActual) {
	...
	echo html_entity_decode('backend/img/data.png');
	die();

} else  {

	//$urlFacebook = "https://www.facebook.com/Informaci%C3%B3n-Polen-Granada-128331124035311/";
	$urMeteoblue = "https://www.meteoblue.com/es/tiempo/outdoorsports/airquality/granada_espa%c3%b1a_2517117";
/*
	$ch = curl_init();
	...
	curl_close($ch);
*/
	$page = file_get_contents($urMeteoblue);
...
echo html_entity_decode('https://salvacam.x10.mx/polen/backend/img/data.png');